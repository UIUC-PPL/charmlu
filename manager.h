...
struct LUMgr : public CBase_LUMgr {
  int blkSize, matSize, numBlks;
  LUMgr(int blkSize_, int matSize_) : blkSize(blkSize_), matSize(matSize_) {
    numBlks = matSize / blkSize;
  }
  virtual int bitsOfPrio() { return 0; }
  virtual CkEntryOptions& setPrio(PrioType type, CkEntryOptions& opts, int y = 0 , int x = 0, int t = 0) = 0;
...
struct PrioLU : public LUMgr {
  ...
  CkEntryOptions& setPrio(PrioType type, CkEntryOptions& opts, int y, int x, int t) {
    int prio = 0;
    switch (type) {
    case SEND_PIVOT_DATA: case DIAG_SEND_PIVOT: prio = -1; break;
    case PROCESS_COMPUTE_U: case RECVL: prio = -1; break;
    case GET_BLOCK: prio = -3; break;
    case SEND_BLOCKS: prio = -2; break;
    case PROCESS_TRAILING_UPDATE:
      if (y == t + 1) prio = 0;
      else prio = t * numBlks * blkSize + y * blkSize;
      break;
    }
  ...
  void setPrio(CkMessage *msg, PrioType type, int refnum, int y, int x) {
    int prio = 0;
    switch (type) {
    case DIAG_SEND_USEG: case BELOW_SEND_USEG: case MULT_RECV_U: case MULT_RECV_L: prio = -1; break;
    case PIVOT_RIGHT_SEC: prio = -1; break;
    case PIVOT_LEFT_SEC: prio = blkSize * numBlks * numBlks + 1; break;
    case PIVOT_CRITICAL: prio = -1; break;
    case PIVOT_NOT_CRITICAL: prio = numBlks * numBlks * blkSize; break;
    }
    *(int*)CkPriorityPtr(msg) = prio;
    CkSetQueueing(msg, CK_QUEUEING_IFIFO);
    // Better way than this?
    if (refnum != -1) CkSetRefNum(msg, refnum);
  }
};
