...
`qsub -A CharmRTS --mode $mode -n $N -t 55 -o run-$runnum-$N-$BLK.out -e run-$runnum-$N-$BLK.err ./charmlu $matrix $BLK $thresh $agglom $mapargs $peRotate $peStride +gz-trace +traceroot run-$runnum-$N-$BLK +logsize 10000 +LBOff`;
